"""Compare legacy and batched owner ACK processing on the same native build.
...
BASELINE_COMMIT = "289bce5f99940f1b96990b725e1bdc1f0b34d624"
...
def paired_rows(rows):
    ...
    for row in rows:
        ...
        pairs.setdefault(key, {})[row["batch_ack"]] = row
    result = []
    for (variant, repetition), pair in pairs.items():
        ...
        result.append({
            ...
            "batch_ack_throughput_rps": on["throughput_rps"],
...
def write_outputs(out, rows):
    comparison.write_csv(out / "witness_batch_ack_runs.csv", rows)
    paired = paired_rows(rows)
    comparison.write_csv(out / "witness_batch_ack_paired.csv", paired)
    summaries = []
    for variant in VARIANTS:
        ...
        for metric in ("legacy_throughput_rps", "batch_ack_throughput_rps",
    ...
    comparison.write_csv(out / "witness_batch_ack_summary.csv", summaries)
    return summaries


def run_parent(args):
    out = (args.output_dir or (
        HERE.parent / "results" / "witness_batch_ack" /
    ...
    for repetition in range(1, args.repetitions + 1):
        ...
        for variant in variants:
            ...
            for enabled in (first, 1 - first):
                case_number += 1
                name = f"rep_{repetition}_{variant}_ack_{enabled}"
                ...
                cmd += ["--batch-ack", str(enabled)]
                env = comparison.b58.child_env(profiling=False)
                env["RAY_RECOVERY_WITNESS_BATCH_ACK"] = str(enabled)
                ...
                if row["batch_ack"] != enabled or row["variant"] != variant:
...
def parser():
    ...
    p.add_argument("--batch-ack", type=int, choices=(0, 1))
...
def main():
    ...
    if args.command == "_single-perf":
        if (args.batch_ack is None or args.single_variant is None
        ...
        os.environ["RAY_RECOVERY_WITNESS_BATCH_ACK"] = str(args.batch_ack)
        row = comparison.b58.single_perf(args)
        if row["witness_batch_ack_enabled"] != bool(args.batch_ack):
            raise RuntimeError("Native ACK mode mismatch: rebuild this commit first")
        row.update(batch_ack=args.batch_ack, method=comparison.method_family(args.single_variant))
